...
Plugin Name: Wordpress Popular Posts Reloaded
Plugin URI: http://wordpress.org/extend/plugins/wordpress-popular-posts-reloaded
Description: Showcases your most popular posts to your visitors on your blog's sidebar. Use Wordpress Popular Posts as a widget or place it anywhere on your theme using <strong>&lt;?php wppr_get_mostpopular(); ?&gt;</strong>
Version: 1.0
...
*/
add_action('widgets_init', 'load_wppr');
function load_wppr() {
    register_widget('WordpressPopularPostsReloaded');
}
/**
...
*/
if (!class_exists('WordpressPopularPostsReloaded')) {
    class WordpressPopularPostsReloaded extends WP_Widget {
        ...
        var $user_ops = array();
        // constructor
        function WordpressPopularPostsReloaded() {
            global $wp_version;
            // widget settings
            $widget_ops  = array(
                'classname' => 'popular-posts',
                'description' => __('The most Popular Posts on your blog.', 'wordpress-popular-posts-reloaded')
            );
            ...
            $this->user_ops          = get_option('wppr_settings_config');
            if (!$this->user_ops) {
                add_option('wppr_settings_config', $wppr_settings_def);
                $this->user_ops = $wppr_settings_def;
            }
            // print stylesheet
            if ($this->user_ops['tools']['css']) {
                add_action('get_header', array(
                    &$this,
                    'wppr_print_stylesheet'
                ));
            }
            /*
            ...
            add_action('wp_ajax_nopriv_wppr_update', array(&$this, 'wppr_ajax_update'));
            
            add_action('wp_head', array(&$this, 'wppr_print_ajax'));
            
            } else {
            
            // add update action, no ajax
            
            add_action('the_content', array(&$this,'wppr_update') );
            
            }
            
            */
            add_action('wp_ajax_nopriv_wppr_update', array(
                &$this,
                'wppr_ajax_update'
            ));
            ...
            add_action('wp_ajax_wppr_clear_cache', array(
                &$this,
                'wppr_clear_data'
            ));
            add_action('wp_ajax_wppr_clear_all', array(
                &$this,
                'wppr_clear_data'
            ));
            ...
            add_action('wppr_cache_event', array(
                &$this,
                'wppr_cache_maintenance'
            ));
            if (!wp_next_scheduled('wppr_cache_event')) {
                $tomorrow = time() + 86400;
                $midnight = mktime(0, 0, 0, date("m", $tomorrow), date("d", $tomorrow), date("Y", $tomorrow));
                wp_schedule_event($midnight, 'daily', 'wppr_cache_event');
            }
            ...
            if (extension_loaded('gd') && function_exists('gd_info') && version_compare(phpversion(), '4.3.0', '>='))
                $this->thumb = true;
            // shortcode
            if (function_exists('add_shortcode')) {
                add_shortcode('wppr', array(
                    &$this,
                    'wppr_shortcode'
                ));
                add_shortcode('WPPR', array(
            ...
            $wppr_ver = get_option('wppr_ver');
            if (!$wppr_ver) {
                add_option('wppr_ver', $this->version);
            } else if (version_compare($wppr_ver, $this->version, '<')) {
                $this->wppr_upgrade();
                update_option('wppr_ver', $this->version);
            }
        }
        // builds Wordpress Popular Posts Reloaded widgets
        function widget($args, $instance) {
            ...
            echo $after_widget . "\n";
            echo "<!-- End Wordpress Popular Posts Reloaded Plugin v" . $this->version . " -->" . "\n";
        }
        ...
        function update($new_instance, $old_instance) {
            ...
            $instance['markup']['wppr-start']        = empty($new_instance['wppr-start']) ? '&lt;ul&gt;' : htmlspecialchars($new_instance['wppr-start'], ENT_QUOTES);
            $instance['markup']['wppr-end']          = empty($new_instance['wppr-end']) ? '&lt;/ul&gt;' : htmlspecialchars($new_instance['wppr-end'], ENT_QUOTES);
        ...
        function form($instance) {
            // set default values			
            $defaults = array(
                'title' => __('Popular Posts', 'wordpress-popular-posts-reloaded'),
                'limit' => 10,
                'range' => 'daily',
                'order_by' => 'comments',
                'post_type' => 'post,page',
...
?>">
    <?php
            _e('Title:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
?>/wp-admin/options-general.php?page=wppr_admin" title="<?php
            _e('What is this?', 'wordpress-popular-posts-reloaded');
?>">?</a>]</small> <br />
  <input id="<?php
            echo $this->get_field_id('title');
?>" name="<?php
...
?>">
    <?php
            _e('Show up to:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>" value="<?php
            echo $instance['limit'];
?>"  class="widefat" style="width:50px!important" />
  <?php
            _e('posts', 'wordpress-popular-posts-reloaded');
?>
</p>
<p>
  <label for="<?php
            echo $this->get_field_id('range');
?>">
    <?php
            _e('Time Range:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>">?</a>]</small><br />
  <select id="<?php
            echo $this->get_field_id('range');
?>" name="<?php
            echo $this->get_field_name('range');
?>" class="widefat">
...
?>>
    <?php
            _e('Last 24 hours', 'wordpress-popular-posts-reloaded');
?>
    </option>
    <option value="weekly" <?php
            if ('weekly' == $instance['range'])
                echo 'selected="selected"';
?>>
    <?php
            _e('Last 7 days', 'wordpress-popular-posts-reloaded');
?>
    </option>
    <option value="monthly" <?php
            if ('monthly' == $instance['range'])
                echo 'selected="selected"';
?>>
    <?php
            _e('Last 30 days', 'wordpress-popular-posts-reloaded');
?>
    </option>
...
?>>
    <?php
            _e('All-time', 'wordpress-popular-posts-reloaded');
?>
    </option>
  </select>
</p>
...
?>">
    <?php
            _e('Sort posts by:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>>
    <?php
            _e('Comments', 'wordpress-popular-posts-reloaded');
?>
    </option>
...
?>>
    <?php
            _e('Total views', 'wordpress-popular-posts-reloaded');
?>
    </option>
...
?>>
    <?php
            _e('Avg. daily views', 'wordpress-popular-posts-reloaded');
?>
    </option>
  </select>
</p>
<fieldset style="width:214px; padding:5px;"  class="widefat">
  <legend>
  <?php
            _e('Posts settings', 'wordpress-popular-posts-reloaded');
?>
  </legend>
  <?php
            if ($this->postRating):
?>
  <input type="checkbox" class="checkbox" <?php
                echo ($instance['rating']) ? 'checked="checked"' : '';
?> id="<?php
...
?>">
    <?php
                _e('Display post rating', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
                echo bloginfo('wpurl');
...
?>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['shorten_title']['active']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('shorten_title-active');
?>" name="<?php
            echo $this->get_field_name('shorten_title-active');
?>" />
  <label for="<?php
            echo $this->get_field_id('shorten_title-active');
?>">
    <?php
            _e('Shorten title', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  <label for="<?php
                echo $this->get_field_id('shorten_title-length');
?>">
    <?php
                _e('Shorten title to', 'wordpress-popular-posts-reloaded');
?>
    <input id="<?php
                echo $this->get_field_id('shorten_title-length');
?>" name="<?php
                echo $this->get_field_name('shorten_title-length');
?>" value="<?php
                echo $instance['shorten_title']['length'];
?>" class="widefat" style="width:50px!important" />
    <?php
                _e('characters', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <br />
  <br />
...
?>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['post-excerpt']['active']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('post-excerpt-active');
?>" name="<?php
            echo $this->get_field_name('post-excerpt-active');
?>" />
  <label for="<?php
            echo $this->get_field_id('post-excerpt-active');
?>">
    <?php
            _e('Display post excerpt', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  <fieldset class="widefat">
    <legend>
    <?php
                _e('Excerpt Properties', 'wordpress-popular-posts-reloaded');
?>
    </legend>
    &nbsp;&nbsp;
    <input type="checkbox" class="checkbox" id="<?php
                echo $this->get_field_id('post-excerpt-format');
?>" name="<?php
                echo $this->get_field_name('post-excerpt-format');
?>" <?php
                echo ($instance['post-excerpt']['keep_format']) ? 'checked="checked"' : '';
?> />
    <label for="<?php
                echo $this->get_field_id('post-excerpt-format');
?>">
      <?php
                _e('Keep text format and links', 'wordpress-popular-posts-reloaded');
?>
    </label>
    <small>[<a href="<?php
                echo bloginfo('wpurl');
...
?>">?</a>]</small><br />
    &nbsp;&nbsp;
    <label for="<?php
                echo $this->get_field_id('post-excerpt-length');
?>">
      <?php
                _e('Excerpt length:', 'wordpress-popular-posts-reloaded');
?>
      <input id="<?php
                echo $this->get_field_id('post-excerpt-length');
?>" name="<?php
                echo $this->get_field_name('post-excerpt-length');
?>" value="<?php
                echo $instance['post-excerpt']['length'];
?>" class="widefat" style="width:30px!important" />
      <?php
                _e('characters', 'wordpress-popular-posts-reloaded');
?>
    </label>
...
<fieldset class="widefat">
  <legend>
  <?php
            _e('Filters:', 'wordpress-popular-posts-reloaded');
?>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
?>/wp-admin/options-general.php?page=wppr_admin" title="<?php
...
?>">
    <?php
            _e('Post type(s):', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <br />
  &nbsp;&nbsp;
...
?>" class="widefat" style="width:150px" />
  <br />
  <br />
  &nbsp;&nbsp;
  <label for="<?php
...
?>">
    <?php
            _e('Category(ies) ID(s):', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <br />
  &nbsp;&nbsp;
...
?>">
    <?php
            _e('Author(s) ID(s):', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <br />
  &nbsp;&nbsp;
...
<fieldset style="width:214px; padding:5px;"  class="widefat">
  <legend>
  <?php
            _e('Thumbnail settings', 'wordpress-popular-posts-reloaded');
?>
  </legend>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['thumbnail']['active']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('thumbnail-active');
?>" name="<?php
            echo $this->get_field_name('thumbnail-active');
?>" />
  <label for="<?php
            echo $this->get_field_id('thumbnail-active');
?>">
    <?php
            _e('Display post thumbnail', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  <label for="<?php
                echo $this->get_field_id('thumbnail-width');
?>">
    <?php
                _e('Width:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <input id="<?php
                echo $this->get_field_id('thumbnail-width');
?>" name="<?php
                echo $this->get_field_name('thumbnail-width');
?>" value="<?php
                echo $instance['thumbnail']['width'];
?>"  class="widefat" style="width:30px!important" <?php
                echo ($this->thumb) ? '' : 'disabled="disabled"';
?> />
  <?php
                _e('px', 'wordpress-popular-posts-reloaded');
?>
  <br />
  <label for="<?php
                echo $this->get_field_id('thumbnail-height');
?>">
    <?php
                _e('Height:', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <input id="<?php
                echo $this->get_field_id('thumbnail-height');
?>" name="<?php
                echo $this->get_field_name('thumbnail-height');
?>" value="<?php
                echo $instance['thumbnail']['height'];
?>"  class="widefat" style="width:30px!important" <?php
                echo ($this->thumb) ? '' : 'disabled="disabled"';
?> />
  <?php
...
<fieldset style="width:214px; padding:5px;"  class="widefat">
  <legend>
  <?php
            _e('Stats Tag settings', 'wordpress-popular-posts-reloaded');
?>
  </legend>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['stats_tag']['comment_count']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('comment_count');
?>" name="<?php
            echo $this->get_field_name('comment_count');
?>" />
  <label for="<?php
            echo $this->get_field_id('comment_count');
?>">
    <?php
            _e('Display comment count', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>">?</a>]</small><br />
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['stats_tag']['views']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('views');
?>" name="<?php
            echo $this->get_field_name('views');
?>" />
  <label for="<?php
            echo $this->get_field_id('views');
?>">
    <?php
            _e('Display views', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>">?</a>]</small><br />
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['stats_tag']['author']) ? 'checked="checked"' : '';
?> id="<?php
...
?>">
    <?php
            _e('Display author', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>">?</a>]</small><br />
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['stats_tag']['date']['active']) ? 'checked="checked"' : '';
?> id="<?php
...
?>">
    <?php
            _e('Display date', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  <fieldset class="widefat">
    <legend>
    <?php
                _e('Date Format', 'wordpress-popular-posts-reloaded');
?>
    </legend>
    <label title='F j, Y'>
...
?>' value='F j, Y' <?php
                echo ($instance['stats_tag']['date']['format'] == 'F j, Y') ? 'checked="checked"' : '';
?> />
      <?php
...
?></label>
    ...
    <label title='Y/m/d'>
      <input type='radio' name='<?php
                echo $this->get_field_name('date_format');
?>' value='Y/m/d' <?php
                echo ($instance['stats_tag']['date']['format'] == 'Y/m/d') ? 'checked="checked"' : '';
?> />
      <?php
...
?></label>
    ...
    <label title='m/d/Y'>
      <input type='radio' name='<?php
                echo $this->get_field_name('date_format');
?>' value='m/d/Y' <?php
                echo ($instance['stats_tag']['date']['format'] == 'm/d/Y') ? 'checked="checked"' : '';
?> />
      <?php
                echo date_i18n('m/d/Y', time());
?></label>
    <br />
    <label title='d/m/Y'>
      <input type='radio' name='<?php
                echo $this->get_field_name('date_format');
?>' value='d/m/Y' <?php
                echo ($instance['stats_tag']['date']['format'] == 'd/m/Y') ? 'checked="checked"' : '';
?> />
      <?php
                echo date_i18n('d/m/Y', time());
?></label>
    <br />
  </fieldset>
  <?php
...
<fieldset style="width:214px; padding:5px;"  class="widefat">
  <legend>
  <?php
            _e('HTML Markup settings', 'wordpress-popular-posts-reloaded');
?>
  </legend>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['markup']['custom_html']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('custom_html');
?>" name="<?php
            echo $this->get_field_name('custom_html');
?>" />
  <label for="<?php
            echo $this->get_field_id('custom_html');
?>">
    <?php
            _e('Use custom HTML Markup', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  ...
  <p style="font-size:11px">
    <label for="<?php
                echo $this->get_field_id('title-start');
?>">
      <?php
                _e('Before / after title:', 'wordpress-popular-posts-reloaded');
?>
    </label>
    <br />
    <input type="text" id="<?php
                echo $this->get_field_id('title-start');
?>" name="<?php
                echo $this->get_field_name('title-start');
?>" value="<?php
                echo $instance['markup']['title-start'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
    <input type="text" id="<?php
                echo $this->get_field_id('title-end');
?>" name="<?php
                echo $this->get_field_name('title-end');
?>" value="<?php
                echo $instance['markup']['title-end'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
  </p>
  <p style="font-size:11px">
    <label for="<?php
                echo $this->get_field_id('wppr_start');
?>">
      <?php
                _e('Before / after Popular Posts:', 'wordpress-popular-posts-reloaded');
?>
    </label>
    <br />
    <input type="text" id="<?php
                echo $this->get_field_id('wppr-start');
?>" name="<?php
                echo $this->get_field_name('wppr-start');
?>" value="<?php
                echo $instance['markup']['wppr-start'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
    <input type="text" id="<?php
                echo $this->get_field_id('wppr-end');
?>" name="<?php
                echo $this->get_field_name('wppr-end');
?>" value="<?php
                echo $instance['markup']['wppr-end'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
  </p>
  <p style="font-size:11px">
    <label for="<?php
                echo $this->get_field_id('post-start');
?>">
      <?php
                _e('Before / after each post:', 'wordpress-popular-posts-reloaded');
?>
    </label>
    <br />
    <input type="text" id="<?php
                echo $this->get_field_id('post-start');
?>" name="<?php
                echo $this->get_field_name('post-start');
?>" value="<?php
                echo $instance['markup']['post-start'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
    <input type="text" id="<?php
                echo $this->get_field_id('post-end');
?>" name="<?php
                echo $this->get_field_name('post-end');
?>" value="<?php
                echo $instance['markup']['post-end'];
?>" class="widefat" style="width:80px!important" <?php
                echo ($instance['markup']['custom_html']) ? '' : 'disabled="disabled"';
?> />
  </p>
...
?>
  <input type="checkbox" class="checkbox" <?php
            echo ($instance['markup']['pattern']['active']) ? 'checked="checked"' : '';
?> id="<?php
            echo $this->get_field_id('pattern_active');
?>" name="<?php
            echo $this->get_field_name('pattern_active');
?>" />
  <label for="<?php
            echo $this->get_field_id('pattern_active');
?>">
    <?php
            _e('Use content formatting tags', 'wordpress-popular-posts-reloaded');
?>
  </label>
  <small>[<a href="<?php
            echo bloginfo('wpurl');
...
?>
  ...
  <p style="font-size:11px">
    <label for="<?php
                echo $this->get_field_id('pattern_form');
?>">
      <?php
                _e('Content format:', 'wordpress-popular-posts-reloaded');
?>
    </label>
    <input type="text" id="<?php
                echo $this->get_field_id('pattern_form');
?>" name="<?php
                echo $this->get_field_name('pattern_form');
?>" value="<?php
                echo htmlspecialchars($instance['markup']['pattern']['form'], ENT_QUOTES);
?>" style="width:204px" <?php
                echo ($instance['markup']['pattern']['active']) ? '' : 'disabled="disabled"';
?> />
  </p>
  <?php
            endif;
...
<?php
        ...
        function wppr_ajax_update() {
            $nonce = $_POST['token'];
            // is this a valid request?
            if (!wp_verify_nonce($nonce, 'wppr-token'))
                die("Invalid token");
        ...
        function wppr_clear_data() {
            $token = $_POST['token'];
            $clear = isset($_POST['clear']) ? $_POST['clear'] : '';
            $key   = get_option("wppr_rand");
            if (current_user_can('manage_options') && ($token === $key) && !empty($clear)) {
                ...
                if ($clear == 'cache') {
                    if ($wpdb->get_var("SHOW TABLES LIKE '$cache'") == $cache) {
                        ...
                        _e('Success! The cache table has been cleared!', 'wordpress-popular-posts-reloaded');
                    } else {
                        _e('Error: cache table does not exist.', 'wordpress-popular-posts-reloaded');
                    }
                } else if ($clear == 'all') {
                    if ($wpdb->get_var("SHOW TABLES LIKE '$table'") == $table && $wpdb->get_var("SHOW TABLES LIKE '$cache'") == $cache) {
                        $wpdb->query("TRUNCATE TABLE $table;");
                        $wpdb->query("TRUNCATE TABLE $cache;");
                        _e('Success! All data have been cleared!', 'wordpress-popular-posts-reloaded');
                    } else {
                        _e('Error: one or both data tables are missing.', 'wordpress-popular-posts-reloaded');
                    }
                } else {
                    _e('Invalid action.', 'wordpress-popular-posts-reloaded');
                }
            } else {
                _e('Sorry, you do not have enough permissions to do this. Please contact the site administrator for support.', 'wordpress-popular-posts-reloaded');
            }
            die();
        }
        // database install
        ...
        function wppr_print_ajax() {
            // if we're on a page or post, load the script
            if ((is_single() || is_page()) && !is_user_logged_in() && !is_front_page()) {
                // let's add jQuery
                wp_print_scripts('jquery');
                // create security token
                $nonce = wp_create_nonce('wppr-token');
...
<!-- End Wordpress Popular Posts v<?php
                echo $this->version;
?> -->
...
<?php
        ...
        function get_popular_posts($instance, $return = false) {
            ...
            if (!is_array($mostpopular) || empty($mostpopular)) { // no posts to show
                $content .= "<p>" . __('Sorry. No data so far.', 'wordpress-popular-posts-reloaded') . "</p>" . "\n";
            } else { // list posts
                // THUMBNAIL SOURCE
                ...
                if (!$this->user_ops || empty($this->user_ops)) {
                    add_option('wppr_settings_config', $user_def_settings);
                    $this->user_ops = $user_def_settings;
                }
                // HTML wrapper
                if ($instance['markup']['custom_html']) {
                    $content .= htmlspecialchars_decode($instance['markup']['wppr-start'], ENT_QUOTES) . "\n";
                } else {
                ...
                foreach ($mostpopular as $p) {
                    ...
                    if ($instance['post-excerpt']['active']) {
                        if ($instance['markup']['pattern']['active']) {
                            ...
                            $excerpt = ": <span class=\"wppr-excerpt\">" . $this->get_summary($p->id, $instance) . "...</span>";
                        }
                    }
                    // STATS
                    // comments
                    if ($instance['stats_tag']['comment_count']) {
                        $stats .= "<span class=\"wppr-comments\">{$comments} " . __('comment(s)', 'wordpress-popular-posts-reloaded') . "</span>";
                    }
                    // views
                    if ($instance['stats_tag']['views']) {
                        $views_text = ' ' . __('view(s)', 'wordpress-popular-posts-reloaded');
                        if ($instance['order_by'] == 'avg') {
                            if ($instance['range'] != 'daily')
                                $views_text = ' ' . __('view(s) per day', 'wordpress-popular-posts-reloaded');
                        }
                        $stats .= ($stats == "") ? "<span class=\"wppr-views\">{$pageviews} {$views_text}</span>" : " | <span class=\"wppr-views\">{$pageviews} {$views_text}</span>";
                    }
                    //author
                    if ($instance['stats_tag']['author']) {
                        $display_name = get_the_author_meta('display_name', $p->uid);
                        $stats .= ($stats == "") ? "<span class=\"wppr-author\">" . __('by', 'wordpress-popular-posts-reloaded') . " {$display_name}</span>" : " | <span class=\"wppr-author\">" . __('by', 'wordpress-popular-posts-reloaded') . " {$display_name}</span>";
                    }
                    // date
                    if ($instance['stats_tag']['date']['active']) {
                        $stats .= ($stats == "") ? "<span class=\"wppr-date\">" . __('posted on', 'wordpress-popular-posts-reloaded') . " {$date}</span>" : " | <span class=\"wppr-date\">" . __('posted on', 'wordpress-popular-posts-reloaded') . " {$date}</span>";
                    }
                    // POST THUMBNAIL
                    if ($instance['thumbnail']['active'] && $this->thumb) {
                        $tbWidth  = $instance['thumbnail']['width'];
                        $tbHeight = $instance['thumbnail']['height'];
                        $thumb    = "<a href=\"" . $permalink . "\" class=\"wppr-thumbnail\" title=\"{$title}\">";
                        if ($this->user_ops['tools']['thumbnail']['source'] == "featured") { // get Featured Image
                            if (function_exists('has_post_thumbnail') && has_post_thumbnail($p->id)) {
                                ...
                                if ($path) {
                                    $thumb .= "<img src=\"" . $this->pluginDir . "/timthumb.php?src={$path}&amp;h={$tbHeight}&amp;w={$tbWidth}\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" alt=\"{$title}\" border=\"0\" class=\"wppr-thumbnail wppr_fi\" />";
                                } else {
                                    $thumb .= "<img src=\"" . $this->default_thumbnail . "\" alt=\"{$title}\" border=\"0\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" class=\"wppr-thumbnail wppr_fi_def\" />";
                                }
                            } else {
                                $thumb .= "<img src=\"" . $this->default_thumbnail . "\" alt=\"{$title}\" border=\"0\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" class=\"wppr-thumbnail wppr_fi_def\" />";
                            }
                            ...
                            if ($path) {
                                $thumb .= "<img src=\"" . $this->pluginDir . "/timthumb.php?src={$path}&amp;h={$tbHeight}&amp;w={$tbWidth}\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" alt=\"{$title}\" border=\"0\" class=\"wppr-thumbnail wppr_fp\" />";
                            } else {
                                $thumb .= "<img src=\"" . $this->default_thumbnail . "\" alt=\"{$title}\" border=\"0\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" class=\"wppr-thumbnail wppr_fp_def\" />";
                            }
                            ...
                            if ($path != "") {
                                $thumb .= "<img src=\"{$path}\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" alt=\"{$title}\" border=\"0\" class=\"wppr-thumbnail wppr_cf\" />";
                            } else {
                                $thumb .= "<img src=\"" . $this->default_thumbnail . "\" alt=\"{$title}\" border=\"0\" width=\"{$tbWidth}\" height=\"{$tbHeight}\" class=\"wppr-thumbnail wppr_cf_def\" />";
                            }
                        }
                        $thumb .= "</a>";
                    }
                    ...
                    if ($instance['markup']['custom_html']) { // build custom layout
                        if ($instance['markup']['pattern']['active']) {
                            ...
                            $content .= htmlspecialchars_decode($instance['markup']['post-start'], ENT_QUOTES) . "{$thumb}<a href=\"{$permalink}\" title=\"{$title}\" class=\"wppr-post-title\">{$title_sub}</a> {$excerpt}{$stats}{$rating}" . htmlspecialchars_decode($instance['markup']['post-end'], ENT_QUOTES) . "\n";
                        }
                    } else { // build regular layout
                        $content .= "<li>{$thumb}<a href=\"{$permalink}\" title=\"{$title}\" class=\"wppr-post-title\">{$title_sub}</a> {$excerpt}<span class=\"post-stats\">{$stats}</span>{$rating}</li>" . "\n";
                    }
                }
                //print_r($posts_data);
                // END HTML wrapper
                if ($instance['markup']['custom_html']) {
                    $content .= htmlspecialchars_decode($instance['markup']['wppr-end'], ENT_QUOTES) . "\n";
                } else {
                    $content .= "\n" . "</ul>" . "\n";
                }
            }
        ...
        */
        ...
        function wppr_textdomain() {
            load_plugin_textdomain('wordpress-popular-posts-reloaded', false, dirname(plugin_basename(__FILE__)));
        }
        // insert Wordpress Popular Posts' stylesheet in theme's head section, just in case someone needs it		
        function wppr_print_stylesheet() {
            if (!is_admin()) {
                if (@file_exists(TEMPLATEPATH . '/wppr.css')) { // user stored a custom wppr.css on theme's directory, so use it
                    $css_path = get_template_directory_uri() . "/wppr.css";
                } else { // no custom wppr.css, use plugin's instead
                    $css_path = plugins_url('style/wppr.css', __FILE__);
                }
                wp_enqueue_style('wordpress-popular-posts-reloaded', $css_path, false);
            }
        }
        ...
        function add_wppr_admin() {
            add_options_page('Wordpress Popular Posts Reloaded', 'WPPR', 'manage_options', 'wppr_admin', array(
                &$this,
                'wppr_admin'
            ));
        }
        // version update warning
        function wppr_update_warning() {
            $msg = '
            <div id="wppr-message" class="error fade">
  <p>' . __('Your Wordpress version is too old. Wordpress Popular Posts Plugin requires at least version 2.8 to function correctly. Please update your blog via Tools &gt; Upgrade.', 'wordpress-popular-posts-reloaded') . '</p>
</div>
';
            echo trim($msg);
        }
        // cache maintenance
        function wppr_cache_maintenance() {
            global $wpdb;
        ...
        // plugin deactivation
        function wppr_deactivation() {
            wp_clear_scheduled_hook('wppr_cache_event');
            remove_shortcode('wppr');
            remove_shortcode('WPPR');
            //delete_option('wppr_ver');
        }
        // shortcode handler
        function wppr_shortcode($atts = NULL, $content = NULL) {
            extract(shortcode_atts(array(
                ...
                'thumbnail_width' => 0,
                'thumbnail_height' => 0,
                'thumbnail_selection' => 'wpprgenerated',
                'rating' => false,
                'stats_comments' => true,
                'stats_views' => false,
                'stats_author' => false,
...
',
            ...
            $shortcode_ops      = array(
                ...
                'markup' => array(
                    'custom_html' => true,
                    'wppr-start' => empty($wppr_start) ? '
                    <ul>
  ' : $wppr_start,
                    'wppr-end' => empty($wppr_end) ? '
</ul>
' : $wppr_end,
                    'post-start' => empty($post_start) ? '
                    <li>;' : $post_start,
                    'post-end' => empty($post_end) ? '</li>
' : $post_end,
        ...
        function wppr_stats() {
            if (function_exists('add_submenu_page'))
                add_submenu_page('index.php', __('Wordpress Popular Posts Stats'), __('Wordpress Popular Posts Stats'), 'manage_options', 'wppr-stats-display', array(
                    &$this,
                    'wppr_stats_display'
                ));
        }
        function wppr_stats_display() {
    ...
    // create tables
    //register_activation_hook('WordpressPopularPostsReloaded', 'wppr_install');
    register_activation_hook(__FILE__, array(
        'WordPressPopularPostsReloaded',
        'wppr_install'
    ));
}
/**
...
// gets views count
// Since 2.0.0
function wppr_get_views($id = NULL) {
    // have we got an id?
...
// gets popular posts
// Since 2.0.3
function wppr_get_mostpopular($args = NULL) {
    $shortcode = '[wppr';
    if (is_null($args)) {
        $shortcode .= ']';
    } else {
...
* Use wppr_get_mostpopular instead.


function get_mostpopular($args = NULL) {
    return wppr_get_mostpopular($args);
}
